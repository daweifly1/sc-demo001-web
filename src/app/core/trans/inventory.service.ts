...
export namespace InventoryServiceNs {
  ...
  export class InventoryServiceClass {
    ...
    // 查询出入库记录列表
    public getInOutRecordList(filter: { filters: {
    ...
    // 仓库库存详情
    public getWarehouseInventoryDetail(filter: { filters: {
        locationCode: string;
        materialCode: string;
      }, pageNum: number, pageSize: number }): Observable<UfastHttpResT<any>> {
      const data = {
    ...
    public stopInventory(id: string): Observable<UfastHttpResT<any>> {
      return this.http.Post('/inventoryCheck/stopCheck', {id: id}, this.defaultConfig);
    }

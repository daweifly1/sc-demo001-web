...
export class MaterialSettingDetailComponent implements OnInit {
  ...
  getItemData = () => {
    ...
    this.materialManageService.getMaterialSettingItem(this.detailId).subscribe((resData) => {
      ...
      this.detailData.materialClassLevel = resData.value.firstLevel + '/' + resData.value.secondLevel + '/' + resData.value.thirdLevel;
      if (this.detailData.materialType === '2') {
        const temp = { name: '选择设备', field: 'deviceName' };
        this.headerFieldList.push(temp);
      }
      this.editMaterialClassId = resData.value.materialClassId;
      this.detailData.taxRate = this.detailData.taxRate + '%';
      this.detailData.planPrice = this.detailData.planPrice + '';
      this.viewUsage();
    }, (error: any) => {
      this.messageService.closeLoading();

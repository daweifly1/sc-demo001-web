...
export class AbnormalOutComponent implements OnInit {
  ...
  public doErpSync(id: string) {
    ...
    public searchTextChange(searchText) {
        this.filters.abnormalNo = searchText;
    }
    ...
    public refresh() {
        this.filters.abnormalNo = '';
        this.filters.outLocation = '';
        this.filters.status = '';
        this.filters.createName = '';
        this.filters.reasonName = '';
        this.filters.agentName = '';
        this.filters.crmOrder = '';
    ...
    public getAbnormalOutDetail(btn: string) {
        ...
        this.abnormalOutService.getAbnormalOutDetail(this.EveryLineId).subscribe((resData: AbnormalOutServiceNs.UfastHttpResT<any>) => {
            ...
            this.printData.data.headerInfo.qrcode = resData.value.headerInfo.abnormalNo;
            if (btn === 'print') {
    ...
    ngOnInit() {
        this.tableConfig = {
            ...
            total: 0,
            loading: false,
            headers: [{ title: '操作', tdTemplate: this.operation, width: 140, fixed: true },
            ...
            { title: '制单时间', field: 'applicationDate', width: 120, pipe: 'date:yyyy-MM-dd HH:mm' },
            { title: '出库状态', field: 'status', width: 80, pipe: 'stockOutStatus' },
            {title: '是否条码管理', field: 'barcodeFlag', width: 100, pipe: 'barcodeManage'},
            { title: '备注', field: 'note', width: 100 },
            {title: 'ERP同步', field: 'isSynsap', width: 80, pipe: 'erpSyncOther'},
            {title: 'ERP同步结果', field: 'isSynsapSuccess', width: 80, pipe: 'erpSyncOtherRes'},

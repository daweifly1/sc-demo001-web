...
export class AddAbnormalOutComponent implements OnInit {
  ...
  /**
   * 出库类型改变*/
  public onStockOutChange(type: string) {
  ...
  public chooseReservoirFun(item: any) {
    this.abnormalOutForm.controls['outArea'].patchValue(item);
    this.leftList = [];
  ...
  public submitWarehouse() {
    ...
    const fieldList = ['amount', 'amountAfterAdjust', 'applyQty', 'barCode', 'deliveryNum', 'enableNum', 'intentionNum',
      'isChecked', 'locationCode', 'materialsDes', 'materialsId', 'materialsNo', 'materialsType', 'price', 'priceSchemeId',
      'priceSchemeName', 'qty', 'requestDeliveryDate', 'sourcePrice', 'unit'];
    this.leftList.forEach((item: any) => {
      const obj = {};
  ...
  handleOk(): void {
    ...
    this.formData.append('areaCode', this.abnormalOutForm.value.outLocation);

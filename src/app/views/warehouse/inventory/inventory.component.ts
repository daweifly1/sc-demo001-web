...
export class InventoryComponent implements OnInit {
  ...
  showPrintModal(id: string, checkOrderNo: string): void {
  ...
  public getInventoryDetail(btn) {
    this.tableConfig.loading = true;
    // 头部信息
    this.inventoryService.getInventoryDetail(this.EveryLineId).subscribe((resData: InventoryServiceNs.UfastHttpResT<any>) => {
      this.tableConfig.loading = false;
      if (resData.code !== 0) {
        this.messageService.showAlertMessage('', resData.message, 'warning');
        return;
      }
      this.printData.data.headerInfo = resData.value;
      this.printData.data.headerInfo.qrcode = resData.value.checkOrderNo;
    }, (error: any) => {
      this.tableConfig.loading = false;
      this.messageService.showAlertMessage('', error.message, 'error');
    });
    ...
    const filter = {
      ...
      pageSize: this.tableConfig.pageSize,
      filters: {
        checkOrderNo: this.chooseCheckOrderNo
      }
    };
    this.tableConfig.loading = true;
    this.inventoryService.getMaterialList(filter).subscribe((resData: InventoryServiceNs.UfastHttpResT<any>) => {
      ...
      this.printData.data.dataList = resData.value.list;
      this.printData.printTempBdDiction = Inventory.printTempBdDiction;
      this.printData.cacheHeaderFootDiction = Inventory.cacheHeaderFootDiction;
      if (btn === PrintFlag.Preview) {

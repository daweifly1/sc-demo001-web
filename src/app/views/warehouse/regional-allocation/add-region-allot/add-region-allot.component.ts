  ...
  */
  ...
  constructor(
    ...
    this.headerFieldList = [
      ...
      { name: '单据状态', field: 'billStatus', pipe: 'billStatus' },
    ...
    if (event.type === RightSideTableBoxNs.SelectedChangeType.Checked) {
      event.list.forEach((item: any, index: number) => {
        ...
        if (!value) {
          ...
          data['_this'] = data;
          data['_lineId'] = Symbol();
          this.materialDataList.push(data);
        }
      });
    } else {
      event.list.forEach((item: any) => {
        const i = this.materialDataList.findIndex(itemValue => itemValue.materialNo === item.materialsNo);
  ...
  public delMaterialOne(materialNo) {
    ...
    for (let i = 0, len = this.materialDataList.length; i < len; i++) {
      if (materialNo === this.materialDataList[i].materialNo) {
        idList.push(this.materialDataList[i][this.materialField]);
        this.materialDataList.splice(i, 1);
        break;
      }
    }
...
*/
  ...
  handleOk(): void {
    ...
    this.formData.append('outLocation', this.formGroup.value.outLocation);
    this.formData.append('outArea', this.formGroup.value.outArea);
    ...
    this.http.request(req).subscribe((event: any) => {
      if (event.type === 4) {
        ...
        this.materialDataList.forEach((item) => {
          this.materialsObj[item.materialsNo] = { amount: item.amount };
        });
        this.formData = new FormData;
        this.messageService.closeLoading();
      }
  ...
  ngOnInit() {
    ...
    const tableHeaders: UfastTableNs.TableHeader[] = [
      ...
      { title: '默认储位', field: 'unit', width: 100 },
      { title: '实际库存', field: 'unit', width: 100 },
    ...
    this.tableConfig = {
      ...
      headers: [
        ...
        { title: '实际库存', tdTemplate: this.inventoryTpl, width: 100 },

...
export class DispatchBillComponent implements OnInit {
  ...
  getDispatchBillList = (pageNum?: number) => {
    ...
    this.dispatchBillService.getDispatchBillList(filter).subscribe((resData: DispatchBillServiceNs.UfastHttpResT<any>) => {
      ...
      this.actionStatus = {};
      this.dispathcBillDataList.forEach((item) => {
        this.actionStatus[item.invoiceNo] = {
          edit: item.deliveryStatus === ButtonState.save,
          print: item.deliveryStatus !== ButtonState.save,
          del: item.deliveryStatus === ButtonState.save,
          delivery: item.deliveryStatus === ButtonState.delivery
        };
      });
    }, (error: any) => {
      this.tableConfig.loading = false;
  ...
  public batchDelivery() {
    ...
    for (let i = 0, len = this.dispathcBillDataList.length; i < len; i++) {
      if (this.dispathcBillDataList[i][this.tableConfig.checkRowField]) {
        if (this.dispathcBillDataList[i].deliveryStatus !== ButtonState.delivery) {
          this.messageService.showToastMessage('非提交状态的订单不可以发货', 'warning');
          return;
        }
  ...
  ngOnInit() {
    this.tableConfig = {
      ...
      { title: '业务实体', field: 'businessEntity', width: 150 },
      ...
      { title: '制单人', field: 'creatorName', width: 180 }
      ]
    };
